from translate.pddl_parser import open as parsePDDL
from translate.pddl.f_expression import Assign
from translate.pddl.conditions import Atom, Conjunction, NegatedAtom
from translate.instantiate import explore
...
def parse_init_state(pddl_domain_name, pddl_file_name):
    ...
    for init in t.init:
        ...
        if Atom != init.__class__:
            print
            "Error: Init condition not an Atom -- " + str(init.__class__)
            return set([])
...
def parse_goal_state(pddl_domain_name, pddl_file_name):
    ...
    if Conjunction != t.goal.__class__:
        print
        "Error: Goal condition must be a conjunction of atoms -- " + str(t.goal.__class__)
        return set([])
    ...
    for g in t.goal.parts:
        if Atom != g.__class__:
            print
            "Error: Goal condition not an Atom -- " + str(init.__class__)
            return set([])
...
def generate_action(action, act):
    print
    "Warning: This code is deprecated. Please report your usage to a developer."
    ...
    for eff in action.effects:
        ...
        if eff.literal.__class__ == NegatedAtom:
            ...
            print
            "Error: Effect isn't an Atom or NegatedAtom: %s" % str(eff.literal.__class__)
            return None
