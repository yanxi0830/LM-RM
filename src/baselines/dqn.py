...
class DQN:
    ...
    def __init__(self, sess, policy_name, learning_params, curriculum, num_features, num_states, num_actions):
        ...
        # create dqn network
        self._create_network(learning_params.lr, learning_params.gamma, learning_params.num_neurons,
                             learning_params.num_hidden_layers)
        # create experience replay buffer
        if self.use_priority:
            self.replay_buffer = PrioritizedReplayBuffer(learning_params.buffer_size,
            ...
            self.beta_schedule = LinearSchedule(learning_params.prioritized_replay_beta_iters,
                                                initial_p=learning_params.prioritized_replay_beta0, final_p=1.0)
        else:
    ...
    def _create_network(self, lr, gamma, num_neurons, num_hidden_layers):
        ...
        with tf.variable_scope(self.policy_name):  # helps to give different names to this variables for this network
            ...
            if self.tabular_case:
                ...
                with tf.variable_scope("q_network") as scope:
                    q_values, q_values_weights = create_net(self.s1, total_features, total_actions, num_neurons,
                                                            num_hidden_layers)
                    if self.use_double_dqn:
                        scope.reuse_variables()
                        q2_values, _ = create_net(self.s2, total_features, total_actions, num_neurons,
                                                  num_hidden_layers)
                with tf.variable_scope("q_target"):
                    q_target, q_target_weights = create_net(self.s2, total_features, total_actions, num_neurons,
                                                            num_hidden_layers)
    ...
    def _train(self, s1, a, r, s2, done, IS_weights):
        if self.use_priority:
            _, td_errors = self.sess.run([self.train, self.td_error],
                                         {self.s1: s1, self.a: a, self.r: r, self.s2: s2, self.done: done,
    ...
    def learn(self):
        if self.use_priority:
            experience = self.replay_buffer.sample(self.learning_params.batch_size,
