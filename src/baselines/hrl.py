...
class MetaController:
    def __init__(self, sess, policy_name, options, option2file, rm, use_rm, learning_params, num_features, num_states,
                 show_print, epsilon=0.1):
        ...
        # create dqn network
        self.neuralnet = MCNet(sess, self.num_actions, self.num_features, policy_name, self.tabular_case,
                               learning_params.use_double_dqn, lr, num_neurons, num_hidden_layers)
...
class MCNet:
    ...
    def __init__(self, sess, num_actions, num_features, policy_name, tabular_case, use_double_dqn, lr, num_neurons,
                 num_hidden_layers):
        self.sess = sess
        ...
        with tf.variable_scope(self.policy_name):  # helps to give different names to this variables for this network
            ...
            if self.tabular_case:
                ...
                with tf.variable_scope("q_network") as scope:
                    q_values, q_values_weights = create_net(self.s1, num_features, num_actions, num_neurons,
                                                            num_hidden_layers)
                    if use_double_dqn:
                        scope.reuse_variables()
                        q2_values, _ = create_net(self.s2, num_features, num_actions, num_neurons, num_hidden_layers)
                with tf.variable_scope("q_target"):
                    q_target, q_target_weights = create_net(self.s2, num_features, num_actions, num_neurons,
                                                            num_hidden_layers)
            ...
            q_max = tf.reduce_max(q_target - 10000000 * (1.0 - self.s2_mask),
                                  axis=1)  # I remove invalid actions by adding a big negative contants to them
    ...
    def learn(self, s1, a, r, s2, s2_mask, done, gamma):
        self.sess.run(self.train,
                      {self.s1: s1, self.a: a, self.r: r, self.s2: s2, self.s2_mask: s2_mask, self.done: done,
                       self.gamma: gamma})
