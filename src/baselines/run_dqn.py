...
def run_dqn_baseline(sess, rm_file, policy_bank, tester, curriculum, show_print):
    ...
    for t in range(num_steps):
        ...
        # Learning
        if policy_bank.get_step(rm_id) > learning_params.learning_starts and policy_bank.get_step(
                rm_id) % learning_params.train_freq == 0:
            policy_bank.learn(rm_id)

        # Updating the target network
        if policy_bank.get_step(rm_id) > learning_params.learning_starts and policy_bank.get_step(
                rm_id) % learning_params.target_network_update_freq == 0:
...
def run_dqn_save_model(alg_name, tester, curriculum, num_times, show_print):
    ...
    for t in range(num_times):
        ...
        policy_bank = PolicyBank(sess, num_actions, num_features, learning_params, curriculum,
