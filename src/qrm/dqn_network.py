...
def create_linear_regression(x, num_input, num_output):
    W = tf.get_variable("w", [num_input, num_output], initializer=tf.constant_initializer(1.0, dtype=tf.float64),
                        dtype=tf.float64)
...
def _add_dense_layer(x, num_input, num_output, add_relu):
    W = tf.get_variable("w", [num_input, num_output],
                        initializer=tf.truncated_normal_initializer(stddev=0.1, dtype=tf.float64), dtype=tf.float64)
