...
from worlds.farm_world import FarmWorldParams


class TesterFarmWorld:
    ...
    def get_task_params(self, task_specification):
        ...
        params = FarmWorldParams(map_file, max_x=self.max_x, max_y=self.max_y, b_radius=self.b_radius,
                                 b_num_per_color=self.b_num_per_color,
                                 use_velocities=self.use_velocities,
                                 ball_disappear=self.ball_disappear)
        return GameParams("farmworld", params)
